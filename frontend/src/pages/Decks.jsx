...
import { Plus, Trash2, Play, BookOpen, Sparkles, X, Loader } from 'lucide-react';

/* Note : F6 (auto-flashcards SM-2 depuis vidéos vues) est désactivé pour le PFE L3.
   Les fichiers backend (autoFlashcardsController, services/autoFlashcards) sont
   préservés pour la perspective d'évolution. Pour réactiver : décommenter les
   routes dans deckRoutes.js et recoder le bloc "decks auto-générés" dans cette page. */

/* Catégories proposées par défaut (datalist) — l'utilisateur peut quand même taper
   librement, mais la suggestion réduit la fragmentation (Maths / mathématiques / Math). */
const DEFAULT_CATEGORIES = [
  'General', 'Mathématiques', 'Informatique', 'Algorithmique', 'Programmation',
  'Réseaux', 'Bases de données', 'Anglais', 'Français', 'Histoire',
  'Économie', 'Management', 'Finance',
];
...
export default function Decks() {
  ...
  const [deckToDelete, setDeckToDelete] = useState(null); // confirmation modal
  ...
  // Catégories existantes (uniques, casse insensible) + suggestions par défaut.
  // Sert de datalist pour proposer l'autocomplétion sans casser la liberté de saisie.
  const categorySuggestions = useMemo(() => {
    ...
    decks.forEach((d) => push(d.category));
  ...
  return (
    <Layout title="Mes decks">
          ...
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div>
              ...
              <input
                ...
                list="deck-categories"
                autoComplete="off"
              />
              <datalist id="deck-categories">
                {categorySuggestions.map((c) => <option key={c} value={c} />)}
              </datalist>
              <p style={{ fontSize: 11, color: '#94A3B8', margin: '4px 0 0' }}>
                Tape pour voir les suggestions ou crée ta propre catégorie.
              </p>
            </div>
      ...
      {/* Modal de confirmation suppression deck */}
      {deckToDelete && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}
          onClick={(e) => e.target === e.currentTarget && !deletingDeck && setDeckToDelete(null)}
        >
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 16, color: '#1e293b' }}>Supprimer ce deck&nbsp;?</div>
                <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>Cette action est définitive.</div>
              </div>
            </div>

            <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '12px 14px', marginBottom: 18 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#1e293b', marginBottom: 4 }}>{deckToDelete.title}</div>
              <div style={{ fontSize: 13, color: '#7F1D1D' }}>
                {deckToDelete.cardCount ?? 0} carte{(deckToDelete.cardCount ?? 0) !== 1 ? 's' : ''} sera{(deckToDelete.cardCount ?? 0) > 1 ? 'ont' : ''} supprimée{(deckToDelete.cardCount ?? 0) > 1 ? 's' : ''} avec le deck.
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              >
                {deletingDeck
                  ? <><Loader size={13} style={{ animation: 'spin 1s linear infinite' }} /> Suppression…</>
